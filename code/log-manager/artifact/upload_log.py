...
class LogManager:
            ...
            for log_filename in logs_file_list:
                ...
                try:
                    ...
                    self.logger.log_error(
                        f"Permission denied while handling log {log_filename}: {e}")
                    continue
                except IOError as e:
                    self.logger.log_error(
                        f"I/O error while handling log {log_filename}: {e}")
                    continue
                except Exception as e:
                    self.logger.log_error(
                        f"Unexpected error while handling log {log_filename}: {e}")
                    continue
    ...
    def _handle_special_log(self, log_dir: str, log_filename: str) -> None:
        ...
        try:
            ...
            try:
                ...
                raise IOError(f"Failed to copy log file {log_filename}: {e}") from e
            ...
            if log_upload_success:
                self.logger.log_info(
                    f"Special log file {timestamped_filename} uploaded successfully.")
                try:
            ...
            raise RuntimeError(f"Failed to handle special log {log_filename}: {e}") from e
    ...
    def _handle_regular_log(self, log_dir: str, log_filename: str, log_type: str) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to handle regular log {log_filename}: {e}") from e
